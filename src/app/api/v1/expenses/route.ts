...
export async function GET(request: NextRequest) {
  try {
    ...
    return v1Json({ expenses, summary });
  } catch {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.text) {
      ...
      if (!expense) return v1Error("Could not parse expense text", 400);
      return v1Json({ expense }, 201);
    }
    if (!body.amount) return v1Error("amount or text is required", 400);
    ...
    return v1Error("Failed to create expense", 500);
  }
}
