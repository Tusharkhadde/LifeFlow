...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!searchQuery) return v1Error("url, title, or question required", 400);
...
${memory || "(nothing)"}`;
    ...
    return v1Json({ answer, related, alreadySaved: alreadySaved ? { id: alreadySaved.id, title: alreadySaved.title } : null });
  } catch {
