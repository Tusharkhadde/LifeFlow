...
export function v1Fail(error: unknown, request: NextRequest, fallback: string, status = 500) {
  if (error instanceof Error && error.message === "Authentication required") {
    return v1Error("Invalid or missing API key", 401, request);
  }
  return v1Error(fallback, status, request);
}
