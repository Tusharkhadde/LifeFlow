...
`chrome-extension/` (load unpacked). Click the toolbar icon to open the side panel. Set app URL + API key once. Features: **Ask my brain** (page text + vault → grounded answer via `/api/v1/copilot`), **Save page**, **Task from page**, **Notes → tasks** (selected text → meeting intelligence).
...
Sends clipped URLs to LifeFlow API (requires user session or configured endpoint).
